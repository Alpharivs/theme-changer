...
def choose_file(mode):
    try:
        ...
        ''' Validate if the file is an actual image and if so copy the selected file into the images folder renaming.. 
        it according to the mode and preserving the extension'''
        ...
        showinfo(title="Error", message="No image was selected")
...
def time_window():
    ...
    # Create input fields
    Label(top, text="Night mode time", font=('Calibri 10')).pack()
    ...
    Label(top, text="Light mode time", font=('Calibri 10')).pack()
...
if __name__ == '__main__':
    ...
    light_button = ttk.Button(
        ...
        command= lambda: choose_file('light') 
    )
    ...
    night_button = ttk.Button(
        ...
        command= lambda: choose_file('night') 
    )
